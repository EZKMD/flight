...
static RouteMapScene cached_scene;
...
static bool rasterize_coastline(RouteMapScene *scene)
{
    size_t segment;
...
static bool same_scene(const RouteMapScene *scene, GeoCoordinate origin,
                       GeoCoordinate destination, int columns, int rows)
{
                             ...
                             const Layout *layout, bool geography_enabled)
{
    ...
    if (layout->mode == LAYOUT_COMPACT || layout->mode == LAYOUT_TINY ||
        layout->height < 14) {
        route_map_compat_render(frame, flight, animation, layout);
        return;
    }
    ...
    if (!same_scene(&cached_scene, origin, destination, columns, rows) &&
        !route_map_scene_prepare(&cached_scene, origin, destination,
                                 columns, rows, true)) {
        frame_center(frame, "ROUTE GEOMETRY UNAVAILABLE", 0);
        return;
    }
    ...
    if (geography_enabled && !cached_scene.geography_available)
        frame_center(frame, "GEOGRAPHY UNAVAILABLE · g TOGGLE", 0);
    else frame_center(frame, geography_enabled ?
                      "GEOGRAPHY ON · g TOGGLE" :
                      "GEOGRAPHY OFF · g TOGGLE", 0);
